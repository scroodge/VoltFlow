...
- Phantom-drain is calculated server-side per UTC day. Never read an unpaginated raw
  ...
  `is_charging` Boolean data. Drain is the positive net SOC loss inside continuous
  parked/unplugged intervals of at least four hours; UTC midnight, movement, charging,
  and telemetry gaps of six hours or more split an interval. Never subtract the whole
  day's first and last SOC because that misclassifies driving consumption as parked drain.
