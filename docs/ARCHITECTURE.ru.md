...
```
      Di+ / autoservice / GPS на головном устройстве DiLink
                              │ локальный state-aware сбор данных
                              ▼
┌───────────────────────────────────────────────────────────────────────────────────┐
│ APK VoltFlow Mate                                                                  │
...
(одна актуальная строка/      (tiered raw-retention,                (выводятся сервером)
...
приложения устарел. GPS он не передаёт и параллельно с sender приложения не работает. Когда
видимый экран VoltFlow выдаёт короткое окно быстрого статуса, оба command-клиента могут вместо
этого отправлять `live_only` snapshot с высокой периодичностью; это меняет только свежесть live-данных,
но не GPS и не постоянную историю.
...
### Быстрый статус на видимом экране

Обычная доставка намеренно собирается в батчи, чтобы ограничить работу backend. Пока пользователь
видит live-экран VoltFlow, PWA продлевает истекающее окно быстрого статуса для конкретного
автомобиля — это **app-owned** состояние в Postgres. Mate получает оставшееся время из уже
существующего command poll и после получения этого окна отправляет `live_only` snapshot с
настроенной периодичностью примерно раз в три секунды. Такие snapshots обновляют только
`bydmate_live_snapshots`; они не создают строки истории, hourly-rollup или поездок. Настроенное
поведение доставки описано в [Telemetry](../supabase/TELEMETRY.md); это не гарантия сквозной
задержки.

Окно можно только продлевать, и оно истекает автоматически; отдельного «выключить» нет. Поэтому
после закрытой вкладки, потери соединения или сбоя клиента следующий command poll возвращает
автомобиль к обычной доставке. Это не пользовательская настройка и её безопасно потерять.
Существующий Realtime subscriber объединяет изменения snapshot на одну секунду; фактическая
свежесть в браузере также зависит от момента, когда command-клиент увидел окно, и от сети.
