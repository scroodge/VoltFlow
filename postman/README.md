...
`src/app/api/**` (40 route files, 49 requests once all exported methods per
...
| `cron_secret` | `cron/inactivity-check` | Must match the server's `CRON_SECRET` env var |
...
3. **Shared-secret header** — `cron/inactivity-check`, `telegram/webhook`.
