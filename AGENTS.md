...
- Mate ingest auto start/stop uses `isAcWallboxCharging` (not raw `is_charging` at 100% / 0 kW) and ignores samples with `device_time` before the active session `started_at`.
