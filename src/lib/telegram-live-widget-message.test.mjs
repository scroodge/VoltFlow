...
import {
  ...
  TELEGRAM_LIVE_RANGE_MAX_AGE_MS,
...
test("live widget omits a stale range from the header", () => {
  const lines = message({
    rangeSampleTime: new Date(NOW_MS - TELEGRAM_LIVE_RANGE_MAX_AGE_MS - 1).toISOString(),
  }).split("\n");
