...
export function snapshotNeedsSohBackfill(telemetry: VoltflowMateTelemetry | null | undefined) {
  return validSohPercent(telemetry?.soh_percent) == null;
}

export function enrichSnapshotWithSoh<T extends Pick<VoltflowMateLiveSnapshotRow, "telemetry">>(
  snapshot: T,
...
export async function fetchLastKnownSohForVehicle(
  ...
  for (const row of data ?? []) {
    const soh = validSohPercent((row.telemetry as VoltflowMateTelemetry | null)?.soh_percent);
...
export async function backfillLiveSnapshotsWithSoh(
  ...
  const enriched: VoltflowMateLiveSnapshotRow[] = [];
