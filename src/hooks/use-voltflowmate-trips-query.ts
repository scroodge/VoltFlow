...
async function fetchLatestVoltflowMateTrips(
...
export function useVoltflowMateTripRealtimeInvalidation() {
...
export function useVoltflowMateTripsQuery(
  date: string,
  vehicleId: string | null,
  enabled = true,
) {
  ...
  return useQuery({
    queryKey: queryKeys.voltflowMateTrips(date, vehicleId),
    queryFn: () => fetchVoltflowMateTrips(date, vehicleId),
    enabled: enabled && Boolean(date) && pageVisible,
    refetchInterval: pageVisible ? TRIP_REFETCH_INTERVAL_MS : false,
  });
}

export function useLatestVoltflowMateTripsQuery(
  vehicleId: string | null,
  limit = 1,
  enabled = true,
  lite = false,
  ...
  return useQuery({
    queryKey: queryKeys.voltflowMateLatestTrips(vehicleId, limit, lite),
    queryFn: () => fetchLatestVoltflowMateTrips(vehicleId, limit, lite),
...
export function useTripMonthDatesQuery(
  ...
  return useQuery({
    queryKey: queryKeys.voltflowMateTripMonthDates(year, month, vehicleId),
