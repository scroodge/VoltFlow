...
## 🟡 Vehicle live loading — modern Next.js client boundaries and Realtime fan-out (partially built 2026-07-28)

> The safe first increment is shipped: a top-level Live-tab dynamic boundary, one trip
> Realtime observer per Dashboard/History/Vehicle view, and a privacy-minimised
> Vehicle-ready RUM event. See [CHANGELOG.md](CHANGELOG.md). The core visualization-module
> extraction and production bundle inspection below remain required before this plan can
> be closed; Live is the default tab, so the outer dynamic boundary alone is not proof
> that chart/map code waits for interaction. Route Insights now also requests its map
> preview only when a user expands a route; this is a safe consumer-level boundary, but
> it does not satisfy the remaining initial `/vehicle` visualization criterion.

### Goal

Make the authenticated `/vehicle` Live tab reach its useful cockpit state with less
initial JavaScript and less duplicate client Realtime work, without changing telemetry
freshness, RLS, URLs, or the vehicle/charging source-of-truth rules.

### Research findings

- `src/components/vehicle/vehicle-live-view.tsx` is a 4,586-line Client Component. It
  owns the initially visible live cockpit and trip summary, but also exports the
  telemetry-history chart and interactive route-map implementations consumed by
  `TripDetailPanel`, History Analytics, route insights, and dev fixtures. Those
  interaction-only visualizations and their helpers therefore share the same client
  module boundary as the initial Live tab. `TripDetailPanel` is dynamic already, but
  dynamic-importing it cannot make code in its parent module lazy.
- The route already defers whole inactive views correctly: Vehicle defers `ServiceView`,
  History defers analytics/trip detail, and Dashboard defers summaries. The missing
  boundary is inside the oversized Vehicle Live module, not another indiscriminate
  `useMemo` pass.
- `useBydmateTripsQuery` and `useLatestBydmateTripsQuery` each call
  `useBydmateTripRealtimeInvalidation`. Vehicle Live mounts both, creating two
  authenticated `bydmate_trips` Realtime channels and two debounced invalidation timers
  for the same three TanStack Query families. Query de-duplication limits duplicate
  fetches, but does not eliminate duplicate subscriptions/callback work.
- `useBydmateLiveQuery` intentionally refetches the authoritative snapshot list after a
  coalesced Realtime event. That preserves SoH backfill and delete/update semantics, but
  it is a measured-next candidate, not a safe blind `setQueryData` replacement: live
  telemetry is user-owned Postgres data and its event ordering must remain correct.
- Installed Next.js 16.2.10 guidance says Client Components and their libraries should
  use top-level `next/dynamic` boundaries when they are not needed initially. Its newer
  Cache Components model is valuable for cacheable server data, but fresh private data
  should stream under Suspense rather than be cached. This Live tab is already a
  realtime Client Component, so enabling `cacheComponents` globally is not an initial
  loading fix for it and would broaden risk across authenticated routes.
- `@vercel/speed-insights` is installed and Dashboard already emits a
  `dashboard_hero_ready` RUM mark. Vehicle Live has no corresponding ready metric, so
  static inspection cannot prove the initial-bundle or interaction-cost improvement.

### Data ownership and location

No user-facing data model or ownership changes are proposed. Live snapshots, trips,
track points, and charging sessions remain **user-owned Postgres data under RLS**.
TanStack Query/Realtime state remains an in-browser, non-canonical read cache; it must
not become an authority or be persisted to `localStorage`. No new preferences,
migrations, or browser storage are proposed.

### Options and trade-offs

1. **Bounded client-boundary slice (recommended).** Extract the chart/map exports and
   their private helpers into feature-local Client modules, update every consumer to
   import those modules directly, and retain top-level `next/dynamic` only where the
   interaction is conditional. Consolidate trip-table invalidation into one observer per
   mounted view. Add a Vehicle-ready RUM mark, then compare initial load and trip-detail
   interaction with the deployed baseline. This cuts the initial Live-tab dependency
   surface without changing data delivery; it requires careful import migration and
   focused fixture/trip checks.
2. **Enable Next.js Cache Components globally now.** This is the newest broad Next.js
   rendering model, but it would require auditing all request-time authentication,
   cookies, Suspense boundaries, and invalidation semantics. It cannot cache current
   private live telemetry and creates a large unrelated migration. Do not choose for
   this performance task.
3. **Apply Realtime events directly to the live-query cache now.** Potentially removes
   post-event read round trips, but must first prove event payload completeness, update
   ordering, delete handling, and the SoH-backfill contract. Defer until RUM/query
   evidence shows the current refetch is material.
4. **Leave the current layout.** Lowest implementation risk, but preserves the oversized
   initial Client-module boundary and duplicate trip subscriptions.

### Recommendation and implementation plan

Build option 1 as a single contract-preserving vertical slice:

1. Establish private vehicle visualization modules for route-map and telemetry-history
   rendering/geometry, leaving the initial `VehicleLiveView` responsible only for live
   state, summary cards, and trip-list expansion. Update History, Trip Detail, route
   insights, and dev fixtures to import the owning visualization entry points directly;
   remove the old competing exports in the same change.
2. Keep `next/dynamic` declarations top-level and load a heavy visualization only when
   its trip, analytics panel, or map is opened. Preserve server rendering unless a
   component truly depends on browser-only APIs; do not put `ssr: false` on an entire
   live route merely to create a split point.
3. Make `useBydmateTripRealtimeInvalidation` mount once per consuming view (or expose
   one combined trip-query hook) while retaining its five-second batching, its open-trip
   filtering rule, and all three invalidated query families.
4. Add a production-safe Vehicle-ready timing event analogous to the existing Dashboard
   event. Inspect emitted RUM and bundle/build analysis before considering direct
   Realtime cache patching or Cache Components elsewhere.
5. Add/adjust focused tests for pure extracted helpers and keep the fixture route,
   trip expansion, selected-vehicle scoping, and live/charging/stale views working.
   Verify with the relevant Node tests, type-check, and a production build only after
   explicit build approval; do not start or restart the existing development server.

### Acceptance criteria

- Initial `/vehicle` Live renders without loading chart/map implementation code until its
  corresponding interaction is requested, confirmed by a production bundle inspection.
- A Vehicle view owns at most one `bydmate_trips` Realtime subscription and preserves
  the existing debounced refresh behavior on trip start/finish.
- Live snapshot freshness, trip URLs, request/response contracts, RLS scope, and charging
  progress ownership are unchanged.
- RUM compares Vehicle-ready timing before/after in production; a direct Realtime cache
  change is not claimed or merged without that evidence.

Proposed 2026-07-28; awaiting explicit go-ahead.
