...
## 🟡 Refactor `admin-users-panel.tsx` (1012 lines, 9 components in one file)

### Problem

`src/components/admin/users/admin-users-panel.tsx` (only importer:
`src/app/admin/users/page.tsx`) mixes data fetching, filter state, and presentation:

- **Fetch/pagination logic inlined in the component.** A 12-dependency `useEffect`
  with hand-rolled `fetch`, no abort (stale responses from an earlier filter can append
  to a newer list — race), and `resetFilters()` must be remembered at every setter.
- **Six filter `useState`s + 3 near-identical pill rows** (`telemetry`, `premium`,
  `lastSeen`) with duplicated `if (x !== f) { resetFilters(); set(f) }` and nested
  ternary labels.
- **`PremiumEditor` (~280 lines)** holds 9 `useState`s, the payments fetch, and the
  submit logic; `PremiumUpdatePayload` is declared inside the component; hard-coded
  input ids (`premium-until`, `payment-*`) collide if two editors are open.
- **Duplicated mutation boilerplate** (`fetch` → `ok` check → toast → `onUpdated`) in
  `PremiumEditor.submit` and `AdminRevoker.revoke`.
- **Formatters** (`relativeTime`, `compactNumber`, `formatDate`, `formatMetricDate`,
  `toDatetimeLocalValue`, `paymentMethodLabel`) and the attention label/tone/detail
  switches are untested and buried in a component file.
- `loading` is set from three places (`resetFilters`, `onUpdated`, effect `finally`).

### Ownership / storage

Not applicable — no data model change; the API contracts (`/api/admin/users*`) and UI
output are unchanged. Behavior-preserving refactor only.

### Options

1. **Split by responsibility (recommended).** Keep the folder `src/components/admin/users/`:
   - `admin-users-panel.tsx` — thin composition (~80 lines).
   - `use-admin-users.ts` — filters object + fetch with `AbortController`,
     infinite-scroll page state; exposes `{users, stats, attention, hasMore, loading,
     filters, setFilter, refresh}`. `setFilter` resets the page in one place.
   - `admin-users-filters.tsx` — data-driven `FilterGroup` (options array with labels)
     replaces the 3 hand-written rows and the nested ternaries.
   - `admin-users-metrics.tsx`, `attention-queue.tsx`, `user-card.tsx`,
     `activity-grid.tsx`, `premium-editor.tsx` (+ `use-premium-payments.ts`),
     `admin-revoker.tsx`.
   - `src/lib/admin-users-format.ts` — pure formatters, plus
     `admin-users-format.test.mjs` (relativeTime, compactNumber, toDatetimeLocalValue);
     attention label/tone/detail move next to `admin-users-attention.ts` (already tested).
   - Shared `postAdminAction()` helper for the fetch/ok/toast pattern; `useId()` for
     input ids.
2. **Minimal: extract components only**, leave the effect and state as is. Smaller
   diff, but keeps the stale-response race and the reset-at-every-setter trap.
3. **Adopt TanStack Query** (`useInfiniteQuery`) — already used in settings hooks. Best
   caching/abort story, but changes loading semantics and adds more surface than a
   refactor should; better as a follow-up.

### Recommendation

Option 1. Behavior stays identical except the race fix (aborted stale requests).
Verify with `npm run lint`, `npm run build`, `npm run test` (new formatter tests), and a
manual pass of `/admin/users`: search, each filter, infinite scroll, attention
"open user", premium save/clear/payment, revoke admin.

### Note

The file has uncommitted local edits (+135/−34: attention queue, metrics tiles). The
refactor would build on that working-tree version, so commit or confirm those first.
