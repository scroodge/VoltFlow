-- Store per-trip distance as a delta from the car trip meter reading at open.
-- Previously distance_km copied current_trip_distance_km as-is, so a new trip
-- after parking inherited the previous trip's meter value (e.g. 94.1 km instead
-- of ~50.5 km for Cl on 2026-06-14).

alter table public.bydmate_trips
  add column if not exists trip_meter_baseline_km numeric;

comment on column public.bydmate_trips.trip_meter_baseline_km is
  'current_trip_distance_km at trip open; distance_km = meter_now - baseline (with reset handling).';

create or replace function public.bydmate_trip_distance_from_meter(
  p_baseline numeric,
  p_current numeric,
  p_existing_distance numeric default null
)
returns table (
  distance_km numeric,
  trip_meter_baseline_km numeric
)
language sql
immutable
as $$
  select
    case
      when p_current is null then p_existing_distance
      when p_baseline is null then 0::numeric
      when p_current < p_baseline then coalesce(p_existing_distance, 0) + p_current
      else p_current - p_baseline
    end as distance_km,
    case
      when p_current is null then p_baseline
      when p_baseline is null then coalesce(p_current, 0)
      when p_current < p_baseline then 0::numeric
      else p_baseline
    end as trip_meter_baseline_km;
$$;
