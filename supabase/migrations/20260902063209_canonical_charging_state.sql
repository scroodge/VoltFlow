-- Canonical charging state for server ingest. An explicit gun reading is authoritative:
-- autoservice first, then DiPlus. DiPlus charging_status survives only for Leopard 3's
-- reduced-payload case where neither gun source is present. Client is_charging is not an
-- authority for server trip state.
create or replace function public.bydmate_canonical_is_charging(p_raw_payload jsonb)
returns boolean
language sql
immutable
security invoker
...
as $$
  with source as (
    select
      coalesce(
        nullif(trim(p_raw_payload #>> '{autoservice,gun_state}'), ''),
        nullif(trim(p_raw_payload #>> '{diplus,charge_gun_state}'), '')
      ) as gun,
      nullif(trim(p_raw_payload #>> '{diplus,charging_status}'), '') as charging_status
  )
  select case
    when gun is not null then gun in ('2', '3', '4', '5')
    when charging_status ~ '^[+-]?[0-9]+(?:\.[0-9]+)?$' then charging_status::numeric > 0
    else false
  end
  from source;
$$;

revoke all on function public.bydmate_canonical_is_charging(jsonb) from public, anon, authenticated;
grant execute on function public.bydmate_canonical_is_charging(jsonb) to service_role;

-- Redefine the current 9-argument ingest implementation with only its trip charging
-- classification changed to call the canonical helper. The body below is copied from
-- 20260720160000_bydmate_client_trip_hot_path.sql to preserve every other hot-path behavior.
...
begin
  ...
  v_is_charging := public.bydmate_canonical_is_charging(p_raw_payload);
