-- Discard stationary micro-trips (common while parked) and avoid creating new trips without drive evidence.
...
begin
  ...
  if v_trip.sample_count >= 3 then
    return false;
  end if;

  if coalesce(v_trip.distance_km, 0) > 0.1 then
    return false;
  end if;

  if coalesce(v_trip.max_speed_kmh, 0) > 3 then
    return false;
  end if;
  ...
  delete from public.bydmate_trips
  where id = p_trip_id;
...
revoke all on function public.bydmate_discard_trip_if_junk(uuid) from public;
...
-- Historical cleanup: stationary micro-trips (often 2 samples while parked).
delete from public.bydmate_trip_track_points
where trip_id in (
  select id
  from public.bydmate_trips
  where sample_count < 3
    and coalesce(distance_km, 0) <= 0.1
    and coalesce(max_speed_kmh, 0) <= 3
);
